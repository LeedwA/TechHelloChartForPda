package com.cwgj.chartlib.hellocharts.renderer;
...
import com.cwgj.chartlib.hellocharts.computator.ChartComputator;
...
import com.cwgj.chartlib.hellocharts.util.AxisAutoValues;
import com.cwgj.chartlib.hellocharts.util.ChartUtils;
import com.cwgj.chartlib.hellocharts.util.FloatUtils;
import com.cwgj.chartlib.hellocharts.view.Chart;
...
//    int rangeColor[];
    ...
    public void drawInBackground(Canvas canvas) {
        ...
        if (null != axis) {
            ...
            drawAxisLines(canvas, axis, LEFT, 0);
        }
    ...
    //    画Y轴
    private void drawAxisLines(Canvas canvas, Axis axis, int position, int i) {
...
//        获取Y轴的区间大小
            float range = chart.getYaxisRange();
//        获取Y轴的区间颜色
            int[] colors = chart.getYRangeColors();
//            如果未设置range或者rangecolors[]则使用默认方法绘制，否则进行分层绘制
            if (range == 0 || colors == null) {
                labelPaintTab[position].setStrokeWidth((float) 5);
                canvas.drawLine(separationX1, separationY1, separationX2, separationY2, labelPaintTab[position]);
            } else {
//        获取Y轴的最大值
                float height = chart.getMaximumViewport().height();
//       前N个区间长度为range,最后一个为height-N*range
                int colorNum = chart.getYRangeColors().length;
                // 计算得到range的个数
                int rangeNum = (int) (height / range);
                float percent = (height % range) / height;
//       获取绘图时的累加分段长度
                float lon = (separationY2 - separationY1) * (1 - percent) / rangeNum;
                Paint paint = new Paint();
                paint.setStrokeWidth((float) 5);
                for (int j = 0; j < colorNum; j++) {
                    paint.setColor(colors[j]);                    //设置画笔颜色
                    canvas.drawLine(separationX1, separationY1 + j * lon, separationX2, separationY1 + (j + 1) * lon, paint);
                    if (j == (colorNum - 1)) {
                        canvas.drawLine(separationX1, separationY1 + j * lon, separationX2, separationY2, paint);
                    }
                }
            }
