...
import com.cwgj.chartlib.hellocharts.model.Column;
import com.cwgj.chartlib.hellocharts.model.ColumnChartData;
...
import com.cwgj.chartlib.hellocharts.provider.ColumnChartDataProvider;
...
//    Y轴分级颜色
    ...
    private void drawSubcolumn(Canvas canvas, Column column, SubcolumnValue columnValue, boolean isStacked) {
        float i = columnValue.getValue();
       int []colors=chart.getYRangeColors();
        float height=chart.getMaximumViewport().height();
        int range=chart.getYaxisRange();
//        如果未设置rangecolors[],则按照默认模式绘制条形，否则按照rangecolors进行绘制
        if (colors==null) {
            canvas.drawRect(new RectF(drawRect.left, drawRect.top + 20, drawRect.right, drawRect.bottom), columnPaint);
        }else {
            canvas.drawRect(new RectF(drawRect.left, drawRect.top + 20, drawRect.right, drawRect.bottom), columnPaint);
            for (int j = 0; j <colors.length ; j++) {
                if (i>(colors.length-1)*range){
                    int num=colors.length;
                    columnPaint.setColor(colors[num-1]);
                    canvas.drawRect(new RectF(drawRect.left, drawRect.top, drawRect.right, drawRect.top + 20), columnPaint);
                }
                if (i>j*range&&i<=(j+1)*range){
                    columnPaint.setColor(colors[j]);
                    canvas.drawRect(new RectF(drawRect.left, drawRect.top, drawRect.right, drawRect.top + 20), columnPaint);
                }
            }
